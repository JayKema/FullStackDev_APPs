...
Sample: ```curl http://127.0.0.1:5000/questions -X POST -H "Content-Type: application/json" -d '{"question":"What's the name of the longest river in Nigeria", "answer": "River Niger","category" :"1", "difficulty":"2"}'```
...
Sample ```curl http://127.0.0.1:5000/search -X POST -H "Content-Type: application/json" -d '{"searchTerm":"whose"}'```
...
Sample``` curl http://127.0.0.1:5000/quizzes -X POST -H "Content-Type: application/json" -d '{"quiz_category":{"type":"History","id":"2"}, "previous_questions":[15]}'``` 

Returns: A random question in the sample format
```
{
  "question": {
    "answer": "Mona Lisa", 
